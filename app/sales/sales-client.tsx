    ...
    <main className="workspace-page">
      ...
      <section className="workspace-content">
        ...
        <section className="data-card">
          <div className="data-toolbar"><div><h2>Invoices</h2><span>{filtered.length} records</span></div><label className="data-search"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search invoice or product" /></label></div>
            ...
            <div className="sales-history-list">
              {filtered.map((sale) => (
                <article key={sale._id}>
                  ...
                  <div><strong>{sale.invoiceNumber}</strong><small>{new Date(sale.createdAt).toLocaleString("en-IN")}</small></div>
